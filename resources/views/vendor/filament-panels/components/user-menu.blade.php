...
{{-- No teleport: opens instantly without waiting on Floating UI reposition to <body> --}}
<x-filament::dropdown
...
>
    ...
    @if ($profileItem?->isVisible() ?? true)
        ...
        >
            <div class="fi-user-menu-identity">
                <div class="fi-user-menu-name">
                    {{ $profileItem?->getLabel() ?? filament()->getUserName($user) }}
                </div>
                @if (filled($user->email ?? null))
                    <div class="fi-user-menu-email">
                        {{ $user->email }}
                    </div>
                @endif
            </div>
        ...
        @if ($hasProfileItem)
            <x-filament::dropdown.list>
                <x-filament::dropdown.list.item
                    ...
                    tag="a"
                >
                    {{ $profileItem?->getLabel() ?? ($profilePage ? $profilePage::getLabel() : null) ?? 'Profile' }}
                </x-filament::dropdown.list.item>
            </x-filament::dropdown.list>
        @endif
